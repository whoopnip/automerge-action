...
async function queryUpdateAndMerge(context, event) {
  logger.trace('Triggered PR:', event.pull_request);
  const { octokit } = context;
  const { data: pr } = await octokit.pulls.get({
    owner: event.pull_request.head.repo.owner.login,
    repo: event.pull_request.head.repo.name,
    pull_number: event.pull_request.number
  });
  logger.trace('Pulled PR:', pr);
...
async function handlePullRequestUpdate(context, eventName, event) {
  ...
  await queryUpdateAndMerge(context, event);
}
